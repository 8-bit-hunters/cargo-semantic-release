...
pub fn fetch_all_commits(repository: &Repository) -> Result<Vec<Commit>, Box<dyn Error>> {
...
mod commit_fetcher_tests {
    ...
    fn compare(result_of_get_commits: &[Commit], expected_commits: &[&str]) -> bool {
        let collected_commit_messages: HashSet<_> = result_of_get_commits
            .iter()
